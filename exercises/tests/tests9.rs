...
mod foo {
    // Use `#[no_mangle]` to prevent name mangling and `pub` to make it accessible
    #[no_mangle]
    pub fn my_demo_function(a: u32) -> u32 {
        a
    }
    
    // If `my_demo_function_alias` is an alias for `my_demo_function`, 
    // you can either implement it as another function that calls `my_demo_function` or 
    // use `#[no_mangle]` and `pub` for it as well if it's defined separately.
    #[no_mangle]
    pub fn my_demo_function_alias(a: u32) -> u32 {
        my_demo_function(a) // Assuming `my_demo_function` is the actual function
    }
}
...
mod tests {
    ...
    #[test]
    fn test_success() {
        // Since the functions are extern, they are unsafe to call.
        // However, we know they are safe in this context.
        unsafe {
